...
const PLAN_C: Record<string, { color: string; bg: string; border: string }> = {
  pro:     { color: "#7c3aed", bg: "rgba(99,102,241,0.10)",  border: "rgba(99,102,241,0.22)"  },
              ...
              <div style={{ padding: "16px 20px" }}>
                {isEdit ? (
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
                      <div style={{ display: "flex", gap: 6, marginTop: 2 }}>
                        <button
                          ...
                          disabled={saving}
                          style={{ display: "flex", alignItems: "center", gap: 5, padding: "7px 14px", borderRadius: 7, background: "rgba(99,102,241,0.14)", border: "1px solid rgba(99,102,241,0.30)", color: "#7c3aed", fontSize: 12, fontWeight: 600, cursor: saving ? "wait" : "pointer", fontFamily: "inherit", opacity: saving ? 0.6 : 1 }}>
                          {saving ? <Loader2 size={12} className="spin" /> : <Check size={12} />}
                          {saving ? "Salvando…" : "Salvar"}
                        </button>
                        <button onClick={() => { setEditing(null); setSaveErr(""); }}
                          style={{ display: "flex", alignItems: "center", justifyContent: "center", width: 32, height: 32, borderRadius: 7, background: "#f4f5fb", border: `1px solid ${C.sep}`, color: C.muted, cursor: "pointer", fontFamily: "inherit" }}>
                          <X size={13} />
                        </button>
                      </div>
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    ...
                    <button
                      onClick={() => openEdit(p)}
                      style={{ display: "flex", alignItems: "center", gap: 6, padding: "6px 12px", borderRadius: 7, background: "#f4f5fb", border: `1px solid ${C.sep}`, color: C.muted, cursor: "pointer", fontSize: 12, fontWeight: 500, fontFamily: "inherit", flexShrink: 0, transition: "color .15s, border-color .15s" }}
                ...
                {!isEdit && p.mrr > 0 && (
                  <div style={{ marginTop: 12 }}>
                    ...
                    <div style={{ height: 4, borderRadius: 2, background: "#1a1a2e" }}>
