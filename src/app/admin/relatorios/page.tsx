...
const C = {
  ...
  sep: "rgba(255,255,255,0.05)", text: "#f4f4f5", sub: "#a1a1aa", muted: "#52525b", r: 10,
};
          ...
          {PERIODS.map(p => (
            <button key={p.key} onClick={() => setPeriod(p.key)}
              style={{ padding: "6px 14px", borderRadius: 8, fontSize: 12, fontWeight: 700, cursor: "pointer", fontFamily: "inherit",
                ...
                background: period === p.key ? "#818cf8" : "rgba(255,255,255,0.03)",
          ...
          <div key={title} style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: C.r, overflow: "hidden" }}>
            ...
            {items.length === 0 ? (
              ...
              <div key={s.id} style={{ display: "flex", alignItems: "center", gap: 12, padding: "11px 18px", borderBottom: i < items.length - 1 ? `1px solid ${C.sep}` : "none" }}>
                ...
                <div style={{ width: 26, height: 26, borderRadius: 6, flexShrink: 0, background: "rgba(99,102,241,0.10)", border: "1px solid rgba(99,102,241,0.18)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 700, color: "#818cf8" }}>
          ...
          {studiosByMonth.map(({ label, count }) => {
            ...
            return (
              <div key={label} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
                ...
                <div style={{ width: "100%", height: barH, borderRadius: 4, background: count > 0 ? "rgba(99,102,241,0.45)" : "rgba(255,255,255,0.05)", border: count > 0 ? "1px solid rgba(99,102,241,0.30)" : `1px solid ${C.sep}` }}/>
