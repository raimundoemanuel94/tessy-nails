            ...
            >
              ...
              {stats?.last && (
                <div style={{ fontSize: 11, color: C.muted, display: 'flex', alignItems: 'center', gap: 4 }}>
                  <span>🕐</span>
                  <span>Último: <strong style={{ color: C.text }}>{stats.last.service_name}</strong> · {new Date(stats.last.appointment_date).toLocaleDateString('pt-BR', { day: '2-digit', month: 'short' })}</span>
                </div>
              )}
              ...
              {client.phone && (
                <a
                  href={`https://wa.me/55${client.phone.replace(/\D/g, '').replace(/^55/, '')}`}
                  target="_blank"
                  rel="noreferrer"
                  onClick={e => e.stopPropagation()}
                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '7px 0', borderRadius: 8, background: 'rgba(37,211,102,.12)', border: '1px solid rgba(37,211,102,.25)', color: '#25D366', textDecoration: 'none', fontSize: 12, fontWeight: 700 }}
                >
