...
const ST:Record<string,{l:string,c:string}>={confirmed:{l:'confirmado',c:C.green},pending:{l:'pendente',c:C.amber},completed:{l:'concluÃ­do',c:C.purple},cancelled:{l:'cancelado',c:C.red}}
...
export default function AgendaPage(){
  ...
  const fmt=(d:string)=>{const dt=new Date(d+'T00:00');return{w:['Dom','Seg','Ter','Qua','Qui','Sex','SÃ¡b'][dt.getDay()],n:dt.getDate()}}
  ...
  return(
    <div style={{display:'flex',flexDirection:'column',gap:16}}>
      <div><h1 style={{fontSize:20,fontWeight:800,color:'#fff',margin:0}}>Agenda</h1><p style={{color:C.muted,fontSize:12,margin:'3px 0 0'}}>Seus horÃ¡rios</p></div>
      ...
      <div style={{background:C.card,border:`1px solid ${C.border}`,borderRadius:12,padding:18}}>
        <div style={{fontSize:14,fontWeight:700,color:C.text,marginBottom:14}}>{new Date(selDate+'T00:00').toLocaleDateString('pt-BR',{weekday:'long',day:'numeric',month:'long'})} â€” {dayApts.length} atendimento(s)</div>
        {dayApts.length===0?<div style={{textAlign:'center',padding:32,color:C.muted}}><div style={{fontSize:28,marginBottom:8}}>ðŸ“…</div><div>Nenhum agendamento neste dia</div></div>:dayApts.map((a,i)=>(
          <div key={a.id} style={{display:'flex',alignItems:'flex-start',gap:14,padding:'14px 0',borderBottom:i<dayApts.length-1?`1px solid ${C.border}`:'none'}}>
            ...
            <div style={{flex:1}}><div style={{fontSize:14,fontWeight:700,color:C.text}}>{a.client_name}</div><div style={{fontSize:12,color:C.muted,marginTop:2}}>{a.service_name}</div>{a.notes&&<div style={{fontSize:11,color:C.amber,marginTop:3}}>âš  {a.notes}</div>}</div>
