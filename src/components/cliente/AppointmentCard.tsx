      ...
      <AnimatePresence>
        {modal && (
          <motion.div initial={{ opacity:0 }} animate={{ opacity:1 }} exit={{ opacity:0 }}
            ...
            style={{ backgroundColor:"rgba(30,26,46,0.6)", backdropFilter:"blur(8px)" }}>
              ...
              <div className="flex flex-col items-center text-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#9B8FC0] leading-relaxed">
                    Você está cancelando <span className="font-bold text-[#2A2440]">{a.service.name}</span>{" "}
                    do dia <span className="font-bold text-[#2A2440]">{format(a.date,"dd/MM")}</span>{" "}
      ...
      <div className={cn(
        ...
        <div className="p-4 space-y-3">
          ...
          <div className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-[#FAF8FF]">
            ...
            {a.status === "completed" && (
              <span className="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-100 text-[8px] font-black text-emerald-700 uppercase tracking-widest">
                ✓ Concluído
              </span>
            )}
          ...
          {a.observation && (
            <div className="rounded-xl border border-[#EDE5FF] bg-[#FAF8FF] p-3">
              <p className="text-[8px] font-black text-[#9B8FC0] uppercase tracking-widest mb-1">Obs.</p>
              <p className="text-[10px] text-[#6B6480] leading-relaxed">{a.observation}</p>
            </div>
          )}

          {/* Ações */}
          {(canCancel || canReschedule || a.status === "pending") && (
            <div className="space-y-2 pt-0.5">
              {a.status === "pending" && !past && (
                <PaymentButton
                  serviceName={a.service.name} price={10}
                  appointmentId={a.id} isDeposit={true}
                  title="Pagar sinal"
                  className="w-full h-10 rounded-2xl font-bold text-sm bg-[#7C5CBF] hover:bg-[#9D7FD4] text-white transition-colors"
                />
              )}
