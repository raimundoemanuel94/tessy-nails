...
export function BottomNav() {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-50 px-5 pb-[calc(1rem+env(safe-area-inset-bottom))] pointer-events-none max-w-lg mx-auto left-0 right-0">
      <nav className="flex h-[62px] items-center rounded-[28px] bg-[#1E1A2E] shadow-2xl shadow-[#1E1A2E]/40 border border-white/5 pointer-events-auto overflow-hidden relative">

        {/* Shimmer line */}
        <div className="absolute top-0 left-[15%] right-[15%] h-px bg-gradient-to-r from-transparent via-[#9D7FD4]/25 to-transparent" />
          ...
          if ("isSpecial" in item && item.isSpecial) {
            return (
              ...
              >
                <motion.div
                  whileTap={{ scale: 0.9 }}
                  className="h-12 w-12 rounded-[18px] bg-[#9D7FD4] flex items-center justify-center shadow-lg shadow-black/20"
                >
