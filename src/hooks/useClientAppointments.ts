/**
 * Hook — agendamentos do cliente em tempo real (onSnapshot).
 * Atualiza automaticamente quando admin confirma/cancela.
 */
...
export interface RealtimeAppointment {
  ...
  service: {
    ...
    priceFormatted: string; durationFormatted: string;
  };
  date: Date;
        ...
        unsubRef.current = appointmentService.subscribeByClientId(
              ...
              .map(a => {
                ...
                return {
                  ...
                  service: {
                    ...
                    priceFormatted: svc ? `R$ ${svc.price.toFixed(2)}` : "A confirmar",
                    durationFormatted: svc ? `${svc.durationMinutes}min` : "—",
                  },
                  date,
                  time: { id: a.id ?? "", time: format(date, "HH:mm", { locale: ptBR }) },
                  status: a.status as RealtimeAppointment["status"],
                  observation: a.notes ?? undefined,
                  createdAt: ensureDate(a.createdAt ?? new Date()),
                };
              });
          ...
          (err) => {
            ...
            setError(code === "permission-denied"
              ? "Sem permissão."
              : "Erro ao sincronizar agendamentos.");
            setLoading(false);
          }
        );
      })
